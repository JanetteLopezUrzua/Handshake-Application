const StudentEventsList = class StudentEventsList {
  ...
  geteventslist() {
    ...
    this.connection.query(
      `select * from company_events left join (select companies.id, name, photo from companies left join companies_photos on 
            companies.id=companies_photos.id) as tb on company_events.company_id=tb.id order by year asc, month asc, day asc`,
      (err, rows) => {

const CompanyContactInfo = class CompanyContactInfo {
  ...
  postcontactinfo() {
    if (this.req.body.id !== undefined) {
      this.connection.query(
        `update companies set email='${this.req.body.email}', phonenumber='${this.req.body.phonenum}' where id='${this.req.body.id}'`,
        (err) => {
